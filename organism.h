...
namespace {
    ...
    eating_each_other(
        ...
        if (can1eat2) {
            if (organism2.plant()) 
                return make_tuple(organism1.after(v1 + v2), organism2.after(0));
            if (v1 > v2)
                return make_tuple(organism1.after(v1 + v2/2), organism2.after(0));
        }
        // It has failed.
        // Now organism2 attempts to eat organism1.
        if (can2eat1) {
            if (organism1.plant()) 
                return make_tuple(organism1.after(0), organism2.after(v1 + v2));
            if (v2 > v1) 
                return make_tuple(organism1.after(0), organism2.after(v1/2 + v2));
        }
        // Organisms can't eat each other, they either 
        // kill each other or do nothing 
        if (can1eat2 && can2eat1)
            return make_tuple(organism1.after(0), organism2.after(0));

        return make_tuple(organism1.after(), organism2.after());
    } // function body
...
encounter(
    ...
    if (organism1.is_dead() || organism2.is_dead()) 
        return make_tuple(organism1.after(), organism2.after(), std::nullopt);
